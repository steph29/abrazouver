...
const testUsers = [
  { email: 'admin@abrazouver.fr', password: 'admin123', nom: 'Admin', prenom: 'Abrazouver' },
  { email: 'jean.dupont@test.fr', password: 'test123', nom: 'Dupont', prenom: 'Jean' },
  { email: 'marie.martin@test.fr', password: 'test123', nom: 'Martin', prenom: 'Marie' },
];
...
async function seed() {
  ...
  try {
    ...
    if (existingUsers[0].n > 0) {
      console.log('⚠️  La table users contient déjà des données. Passage des utilisateurs.');
    } else {
      for (const u of testUsers) {
        ...
        console.log(`   ✓ Utilisateur créé : ${u.email} (mot de passe: ${u.password})${isAdmin ? ' [ADMIN]' : ''}`);
      }
    ...
    if (existingItems[0].n > 0) {
      console.log('⚠️  La table items contient déjà des données. Passage des items.');
    } else {
